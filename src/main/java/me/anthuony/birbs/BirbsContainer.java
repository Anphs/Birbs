...
public class BirbsContainer implements Runnable {
    ...
    private final Color textUIColor = new Color(255, 255, 255, 255);
    ...
    private Input input;
    private final String title = "Birbs";
...
//					System.out.println("FPS: " + fps);
            ...
            if (render) {
//				renderer.clear();
                renderTime = world.render(this, renderer);
                window.update();
                frames++;
            } else {
                try {
    ...
    public Entity getRandomUniqueEntity(List<Birb> list, List<Entity> exclusionList) {
        ...
        while (exclusionList.contains(randomEntity) && randomEntity.getType() == 1) {
            randomEntity = list.get((int) (Math.random() * list.size()));
        }
        return randomEntity;
    }
